...
class MealyMachine:
    ...
    def draw_minimized_mealy_machine(self, minimized_mealy_machine: dict, output_filename: str):
        state_transition_pairs = []
        state_transition_pairs_with_input_output_signals = {}
        ...
        for state in self.mealy_machine_data.keys():
            net.add_node(state, label=f'Не минимизированный Мили {state}')
        ...
        for state, transitions in minimized_mealy_machine.items():
            for i in range(len(transitions)):
                target_state = transitions[i].split('/')[0]
                output_signal = transitions[i].split('/')[1]
        ...
        for state in minimized_mealy_machine.keys():
            net.add_node(state, label=f'Минимизированный Мили {state}')
