class State:
    ...
    def __init__(self):
        self.transitions = {}  # Переходы в формате {символ: [список состояний]}
        self.epsilon_transitions = []  # Переходы по ε
...
class NFA:
    ...
    @staticmethod
    def for_empty():
        """Создать НКА для пустого множества."""
        start = State()
        accept = State()
        return NFA(start, accept)

    def union(self, other):
        """Объединение двух НКА (операция |)."""
        start = State()
        accept = State()
...
# Парсер регулярного выражения с использованием стека
class RegexToNFA:
    ...
    def nfa_to_table(self, nfa):
        """Преобразовать НКА в табличный формат."""
        states = set()
        transitions = {}
        ...
        state_to_name = {state: f"q{i}" for i, state in enumerate(states)}
        table = [[""] + [state_to_name[s] for s in states]]
